angular.module('suppletter.controller', ['suppletter.service', 'common.service', 'global_variable'])

	.controller('suppletterCtrl', ['$rootScope','$scope', '$ionicPopup', '$state', '$timeout','APP_EVENTS','$ionicHistory','suppletterService','Message',
		function($rootScope,$scope, $ionicPopup, $state, $timeout,APP_EVENTS,$ionicHistory,suppletterService,Message) {
	...
	$scope.submitProc=function(){
			...
			if ($scope.flow.dealOption) {
				suppletterService.commit($scope.flow,$rootScope.userInfo.loginName).then(function(response){
...
/*         		//流程信息
         		suppletterService.flowNodeInfo(parameter).then(function(response){
