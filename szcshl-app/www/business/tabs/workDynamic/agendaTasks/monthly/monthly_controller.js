angular.module('monthly.controller', ['monthly.service', 'common.service', 'global_variable'])

	.controller('monthlyCtrl', ['$rootScope','$scope', '$ionicPopup', '$state', '$timeout','APP_EVENTS','$ionicHistory','monthlyService','Message',
		function($rootScope,$scope, $ionicPopup, $state, $timeout,APP_EVENTS,$ionicHistory,monthlyService,Message) {
	...
	$scope.submitProc=function(){
			...
			if ($scope.flow.dealOption) {
				monthlyService.commit($scope.flow,$rootScope.userInfo.loginName).then(function(response){
         ...
         /*		//流程信息
         		monthlyService.flowNodeInfo(parameter).then(function(response){
