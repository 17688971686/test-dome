angular.module('archives.controller', ['archives.service', 'common.service', 'global_variable'])

	.controller('archivesCtrl', ['$rootScope','$scope', '$ionicPopup', '$state', '$timeout','APP_EVENTS','$ionicHistory','archivesService','Message',
		function($rootScope,$scope, $ionicPopup, $state, $timeout,APP_EVENTS,$ionicHistory,archivesService,Message) {
	...
	$scope.submitProc=function(){
			...
			if ($scope.flow.dealOption) {
				archivesService.commit($scope.flow,$rootScope.userInfo.loginName).then(function(response){
         ...
         /*		//流程信息
         		archivesService.flowNodeInfo(parameter).then(function(response){
