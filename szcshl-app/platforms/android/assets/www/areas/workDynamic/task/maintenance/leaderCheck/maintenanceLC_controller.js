angular.module('maintenanceLC.controller', ['maintenanceProcess.service','common.service', 'ngCordova', 'angularBMap','file.service'])

	//待办工作列表控制器
	.controller('maintenanceLCCtrl', ["$scope", "$state", "maintenanceProcessService", 'GlobalVariable', 'HttpUtil', 'fileSvc', 'Message',
		function($scope, $state, maintenanceProcessService, GlobalVariable, HttpUtil, fileSvc, Message) {
			...
			$scope.submitProc = function(){
				maintenanceProcessService.update($scope, function() {
					var workJson = {
						...
						dealSuggestion: $scope.model.leaderCheckCompleteMsg,
			...
			function init() {
				maintenanceProcessService.get($scope,function(data){
					if( $scope.model.isNeedLeaderCheck==0){
						$scope.model.leaderCheckCompleteMsg="确认通过";
					}else if($scope.model.isNeedLeaderCheck==1){
						$scope.model.leaderCheckCompleteMsg="确认不通过，需返修";
					}
				});
			};
		}
