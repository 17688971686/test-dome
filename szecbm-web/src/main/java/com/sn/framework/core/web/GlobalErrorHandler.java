...
public class GlobalErrorHandler {
    ...
    public ModelAndView getErrorView(HttpServletResponse response, HttpStatus status, String error) {
        if (SNKit.isJsonContent(request)) {
            SNKit.printJsonMsg(response, status, new Result(Result.Status.ERROR, error));
            return null;
        } else {
    ...
    public ModelAndView handleServiceException(ConstraintViolationException e, HttpServletResponse response) {
        logger.error("参数验证失败", e);
        Set<ConstraintViolation<?>> violations = e.getConstraintViolations();
        ConstraintViolation<?> violation = violations.iterator().next();
        String message = violation.getMessage();
        return getErrorView(response, HttpStatus.BAD_REQUEST, message);
    }
    ...
    public ModelAndView handleValidationException(ValidationException e, HttpServletResponse response) {
        logger.error("参数验证失败", e);
        return getErrorView(response, HttpStatus.BAD_REQUEST, "参数验证失败");
    }
