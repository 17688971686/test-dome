var gulp = require('gulp'), jshint = require('gulp-jshint'), uglify = require('gulp-uglify'), rename = require('gulp-rename'), clean = require('gulp-clean'), concat = require('gulp-concat'), plumber = require('gulp-plumber'), notify = require('gulp-notify')// ,
...
gulp.task('scripts', function() {
			return gulp.src('src/main/webapp/contents/app/**/*.js') // 引入所有需处理的JS
					.pipe(plumber()).pipe(jshint.reporter('default')) // S代码检查
...
gulp.task('clean', function() {
			return gulp.src(['src/main/webapp/contents/app-dist'], {
						read : false
					}).pipe(plumber()).pipe(clean());
		});
...
gulp.task('watch', function() {
	...
	gulp.watch('src/main/webapp/contents/app/**/*.js', ['scripts']);
		...
		// gulp.watch(['src/main/webapp/contents/app-dist/**']).on('change',
		// function(file) {
