...
function AcceptAllRevisions() {
    try{
        var webObj=document.getElementById("WebOffice");
        document.all.WebOffice.SetTrackRevisions(4);
    }catch(e){
        alert("异常\r\nError:"+e+"\r\nError Code:"+e.number+"\r\nError Des:"+e.description);
    }
}
...
function unAcceptAllRevisions() {
    try{
        ...
        for(var i=1;i<=vCount;i++){
            ...
            document.all.WebOffice.AcceptRevision(strUserName ,1)
        }
    }catch(e){
        alert("异常\r\nError:"+e+"\r\nError Code:"+e.number+"\r\nError Des:"+e.description);
    }
...
function GetRevAllInfo() {
    var vCount;
    vCount = document.all.WebOffice.GetRevCount();
    var vOpt = 0;
    var vDate;
    for(var i=1; i<= vCount; i++){
        vOpt = document.all.WebOffice.GetRevInfo(i,2);
        if("1" == vOpt){
            vOpt = "插入";
        }else if("2" == vOpt){
            vOpt = "删除";
        ...
        vDate = new String(document.all.WebOffice.GetRevInfo(i,1));
        ...
        alert("用户:"+document.all.WebOffice.GetRevInfo(i,0) + "\r\n操作:" + vOpt + "\r\n内容:" + document.all.WebOffice.GetRevInfo(i,3));
    }
}
/****************************************************
 *
...
function addBookmark() {
    ...
    document.all.WebOffice.SetFieldValue("test", "加入书签test", "::ADDMARK::");


}
function taohong(){
    alert("给名为test的书签添加套红信息：北京点聚信息技术有限公司");
    document.all.WebOffice.SetFieldValue("test", "http://127.0.0.1:83/test.doc", "::FILE::");
}
/****************************************************
 *
 *					填充模板
...
function bToolBar_New_onclick() {
    try{
        var webObj=document.getElementById("WebOffice");
        var vCurItem = document.all.WebOffice.HideMenuItem(0);
...
/*************************************************
 ...
 列子：
 1.通过VBA获取当前用户的用户名
 document.all.WebOffice.GetDocumentObject().Application.UserName;
 2.获取文档的标题
 document.all.WebOffice.GetDocumentObject().FullName;
 **************************************************/
...
function AcceptRevision_onclick() {
    try{
        ...
        document.all.WebOffice.AcceptRevision(strUserName ,0)
    }catch(e){
        alert("异常\r\nError:"+e+"\r\nError Code:"+e.number+"\r\nError Des:"+e.description);
    }
}
...
function hideSeal(){
    var obj;
    try{
        obj = new Object(document.all.WebOffice.GetDocumentObject());
        if(obj !=null){
...
function write2(){
    var obj1;
    try{
        obj1 = new Object(document.all.WebOffice.GetDocumentObject());
        if(obj1 !=null){
...
function ChangeFace(kind){
    if(kind==1){
        ...
        document.all.WebOffice.SetCustomColour(2,250*256*256+244*256+243);
        document.all.WebOffice.SetCustomColour(1,236*256*256+226*256+225);
        document.all.WebOffice.SetCustomColour(3,181*256*256+151*256+153);
        document.all.WebOffice.SetCustomColour(0,247*256*256+243*256+243);
    }
    if(kind==2){
        document.all.WebOffice.SetCustomColour(2,59*256*256+97*256+131);
        document.all.WebOffice.SetCustomColour(1,138*256*256+216*256+239);
        document.all.WebOffice.SetCustomColour(3,53*256*256+94*256+133);
        document.all.WebOffice.SetCustomColour(0,52*256*256+92*256+130);
    }
    if(kind==3){
        document.all.WebOffice.SetCustomColour(2,254*256*256+235*256+220);
        document.all.WebOffice.SetCustomColour(1,249*256*256+219*256+196);
        document.all.WebOffice.SetCustomColour(3,228*256*256+174*256+136);
        document.all.WebOffice.SetCustomColour(0,249*256*256+214*256+191);
...
function SetCustomToolBtn(index,name){
    ...
    document.all.WebOffice.SetCustomToolBtn(index,name);
...
function hideAll(pcExcludeBar1,pcExcludeBar2,pcExcludeBar3,pcExcludeBar4){
    document.all.WebOffice.HideMenuArea(pcExcludeBar1,pcExcludeBar2,pcExcludeBar3,pcExcludeBar4);
}

function UnActiveExcel(){
    document.all.WebOffice.UnActiveExcel();
}

function GetFileBase64(){
    var v=document.all.WebOffice.GetFileBase64("",0);
...
function SaveBinaryFileFromBase64(){
    var tempPath=document.all.WebOffice.GetTempFilePath();//获取临时文件路径
    var v=document.all.FileBase64.value;
    alert("文档存到本地临时路径："+tempPath);
    document.all.WebOffice.SaveBinaryFileFromBase64(tempPath,v);
