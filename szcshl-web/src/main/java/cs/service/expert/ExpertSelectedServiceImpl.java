...
public class ExpertSelectedServiceImpl implements ExpertSelectedService {
    ...
    public PageModelDto<ProjectReviewCostDto> findProjectRevireCostBak(ProjectReviewCostDto projectReviewCostDto, String skip, String size) {
        ...
        sqlBuilder.setParam("signState1", Constant.EnumState.STOP.getValue());
        sqlBuilder.setParam("signState2", Constant.EnumState.DELETE.getValue());
        sqlBuilder.append(" AND (sdk.isassistproc is null or sdk.isassistproc = :isassistproc ) ");
        sqlBuilder.setParam("isassistproc", Constant.EnumState.NO.getValue());
        sqlBuilder.append(" AND (CFM.ID is null or (CFM.CHARGETYPE = :chargeType AND CFM.CHARGENAME = '专家评审费' AND CFM.PAYMENTDATA IS NOT NULL ");
        sqlBuilder.setParam("chargeType", Constant.EnumState.PROCESS.getValue());
        ...
        sqlBuilder1.setParams(sqlBuilder.getParams());
        sqlBuilder1.setValues(sqlBuilder.getValues());
        sqlBuilder1.setTypes(sqlBuilder.getTypes());
        sqlBuilder1.append(" )");
        ...
        sqlBuilders.setParams(sqlBuilder.getParams());
        sqlBuilders.setValues(sqlBuilder.getValues());
        sqlBuilders.setTypes(sqlBuilder.getTypes());
        ...
        if (Validate.isList(projectReviewCostList)) {
            for (int i = 0; i < projectReviewCostList.size(); i++) {
                ProjectReviewCostDto projCostDto = new ProjectReviewCostDto();
                projCostDto.initData(projectReviewCostList.get(i));
                projectReviewCostDtoList.add(projCostDto);
            }
        }
