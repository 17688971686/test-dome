...
//        criteria.add(Restrictions.eq(Policy_.standardPId.getName() , standardId));
    ...
    public void deletePolicy(String idStr) {
        if (Validate.isString(idStr)) {
            HqlBuilder sql = HqlBuilder.create();
            sql.append(" delete from cs_policy  where id in (:deleteId) ");
            sql.setParam("deleteId", idStr.replaceAll(",", "','"));
            sql.append(" or standardPId in (:standardPId)");
            sql.setParam("standardPId", idStr.replaceAll(",", "','"));
            this.executeSql(sql);
        }
    }
